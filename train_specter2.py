...
def load_train_val_test(task: str):
    ...
    #
    # Each spec carries `min_votes` (NHIỆM VỤ 4): how many of the 3-model
    # GPT-5 panel must vote YES for a paper to be included as augmented label.
    # 3 = unanimous (strict precision, for rare classes Special edu/ECE/TVET/LLL)
    # 2 = majority (more permissive, for underperforming Fields where we need
    #     more training samples — accepts slightly noisier data per
    #     UPGRADE_ROADMAP_v2 NHIỆM VỤ 4 design)
    augment_specs = []
    if task == "fields":
        # Special education (original — unanimous)
        if config.SPECIAL_EDU_AUGMENT.exists():
        ...
        # 4 new Fields (NHIỆM VỤ 4 — majority)
        field_idx_lookup = {f: i for i, f in enumerate(config.FIELDS_12)}
        for field_name in ("test and assessment", "curriculum",
                            "Non-STEM Education", "Education economically"):
            aug_path = config.FIELDS_AUGMENT_OUTPUTS.get(field_name)
            if aug_path is not None and aug_path.exists():
                augment_specs.append({
                    "label_value": field_name,
                    "list_col": "fields_list",
                    "binary_col": f"field_{field_idx_lookup[field_name]:02d}",
                    "aug_path": aug_path,
                    "min_votes": 2,
                })
    if task == "levels":
    ...
    def _recount_n_yes_from_votes_json(aug_df: pd.DataFrame) -> pd.Series:
        """Recount actual yes votes per row from the votes_json column.

        Backward-compat workaround: parquets written by an earlier version of
        llm_augment.py have an `n_yes` column that conflates "all VALID votes
        agreed" with "all 3 panel members agreed" — i.e., a paper where 1/3
        models said yes and 2/3 returned invalid JSON would be saved with
        n_yes=1, agreement='unanimous_yes'. The new min_votes filter needs
        the TRUE count of yes votes (0-3).

        Reads votes_json (canonical per-vote record), counts entries where
        either 'is_match' or 'is_special_education' is True. Used at load
        time only; the parquet on disk is left untouched.
        """
        ...
        def _count(s):
            ...
            for v in votes:
                if not isinstance(v, dict):
                    continue
                # response_field name varies across specs (is_match vs
                # is_special_education); accept either.
                for key in ("is_match", "is_special_education"):
                    if v.get(key) is True:
                        n += 1
                        break
            return n

        if "votes_json" in aug_df.columns:
            return aug_df["votes_json"].apply(_count)
        return aug_df.get("n_yes", pd.Series([0] * len(aug_df)))

    train_year_set = set(config.TRAIN_YEARS)
    for spec in augment_specs:
        aug = pd.read_parquet(spec["aug_path"])
        # Filter by recounted-from-votes_json yes count >= min_votes.
        # Recounting from votes_json works correctly for BOTH the legacy
        # parquets (where the stored n_yes was buggy) and any new parquets
        # generated by future llm_augment.py runs.
        min_votes = spec.get("min_votes", 3)
        recounted_n_yes = _recount_n_yes_from_votes_json(aug)
        aug_yes = aug[recounted_n_yes >= min_votes]
        if len(aug_yes) == 0:
            continue
        ...
        vote_tag = f"min_votes={min_votes}/3"
        print(f"Augmenting {spec['label_value']} ({vote_tag}): +{n_train_aug} papers (train only)"
